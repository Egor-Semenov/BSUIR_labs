...
int menu() {
    ...
    cout << "5-Поиск" << endl;
    cout << "6-Вывод данных файла на данный момент" << endl;
    cout << "7-Вывод результата" << endl;
    cout << "8-Сохранить результат в файл" << endl;
    cout << "9-Выход" << endl;
    int i;
    cin >> i;
    return i;
...
int p_dv(passengers a[], int n)
{
    ...
    double x;
    cout << "Введи вес груза: ";
    cin >> x;

    int i = 0, j = n - 1, m;
    while (i < j)
    {
        if (a[i].laggedge_weight == a[j].laggedge_weight)
            if (a[i].laggedge_weight == x) { cout << a[i].surname;  return i;}
            else return -1;

        m = i + (j - i) * (x - a[i].laggedge_weight) / (a[j].laggedge_weight - a[i].laggedge_weight);

        if (a[m].laggedge_weight == x) { cout << a[m].surname; return m;}
        else
            if (x > a[m].laggedge_weight)    i = m + 1;
            else     j = m - 1;
    }
...
int main()
{
    ...
    while (true) {
        ...
        case 5:p_dv(pas, n); break;
        case 6:openfile(); break;
        case 7:result(); break;
        case 8:resultfile(); break;
        case 9:return 0;
